...
const BuyNow: React.FC = () => {
  ...
  const handlePlaceOrder = async(e: React.FormEvent) => {
    ...
    if(!productsArr){
      toast({ title: "Add some products to cart", variant: "destructive"});
      return;
    }
    if(!phone){
      toast({ title: "Please provide your number", variant: "destructive"});
      return;
    }
    if(phone.length > 11){
    ...
    if(paymentMethod === "visa"){
      ...
      if(!cardNumber){
        toast({ title: "Please input visa card number", variant: "destructive"});
        return;
      }
      if(!expMonth){
        toast({ title: "Please input expiry month of card", variant: "destructive"});
        return;
      }
      if(!expYear){
        toast({ title: "Please input expiry year of card", variant: "destructive"});
        return;
      }
      if(!ccv){
        toast({ title: "Please input visa card number", variant: "destructive"});
    ...
    if(paymentMethod === "mastercard"){
      if(!cardNumber){
        toast({ title: "Please input mastercard card number", variant: "destructive"});
        return;
      }
      if(!expMonth){
        toast({ title: "Please input expiry month of card", variant: "destructive"});
    ...
    if(paymentMethod === "paypal"){
      if(!paypalEmail){
        toast({ title: "Please input your paypal email", variant: "destructive"});
        return;
      }
    }
    if(paymentMethod === "maya"){
      if(!mayaPhone){
        toast({ title: "Please input your maya account number", variant: "destructive"});
        return;
      }
    }
    if(paymentMethod === "gcash"){
      if(!gcashPhone){
        toast({ title: "Please input your gcash number", variant: "destructive"});
        return;
      }
    }
    if(!shippingAddress){
      toast({ title: "Please input your complete address", variant: "destructive"});
      return;
    }
    if(!addressLine1){
      toast({ title: "Please input your address line 1", variant: "destructive"});
      return;
    }
    if(!city){
      toast({ title: "Please input your city", variant: "destructive"});
      return;
    }
    if(!state){
      toast({ title: "Please input your state", variant: "destructive"});
      return;
    }
    if(!country){
      toast({ title: "Please input your country", variant: "destructive"});
      return;
    }
    if(!zipCode){
      toast({ title: "Please input your zipcode", variant: "destructive"});
      return;
    }
